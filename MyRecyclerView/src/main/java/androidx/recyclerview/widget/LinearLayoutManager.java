...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements
    ...
    static final String TAG_MSH = "MSH_LinearLayoutManager";

    public void log(String any) {
        Log.e(TAG_MSH, any);
    }

    private static final String TAG = TAG_MSH;
    ...
    /**
     * While trying to find next view to focus, LayoutManager will not try to scroll
     * more
     * than this factor times the total space of the list. If layout is vertical,
     * total space is the
     * height minus padding, if layout is horizontal, total space is the width minus
     * padding.
     */
     ...
     * It does not keep state after layout is complete but we still keep a reference
     * to re-use
     * the same object.
     */
    ...
    /**
     * Many calculations are made depending on orientation. To keep it clean, this
     * interface
    ...
    /**
     * This keeps the final values for how LayoutManager should start laying out
     * views.
     * It is calculated by checking {@link #getReverseLayout()} and View's layout
     * direction.
     ...
     * {@link android.widget.AbsListView#setStackFromBottom(boolean)} and
    ...
    /**
     * Works the same way as
     * {@link android.widget.AbsListView#setSmoothScrollbarEnabled(boolean)}.
    ...
    /**
     * Used to keep the offset values when
     * {@link #scrollToPositionWithOffset(int, int)} is
     * called.
     */
     ...
     * Anchor position and coordinate defines the reference point for LLM while
     * doing a layout.
     */
    ...
    // Reusable int array to be passed to method calls that mutate it in order to
    // "return" two ints.
    // This should only be used used transiently and should not be used to retain
    // any state over
    // time.
    ...
    /**
     * Returns whether LayoutManager will recycle its children when it is detached
     * from
     * RecyclerView.
     *
     * @return true if LayoutManager will recycle its children when it is detached
     * from
     * RecyclerView.
     */
     ...
     * RecyclerView.
     * <p>
     * If you are using a {@link RecyclerView.RecycledViewPool}, it might be a good
     * idea to set
     * this flag to <code>true</code> so that views will be available to other
     * RecyclerViews
     * immediately.
     * <p>
     * Note that, setting this flag will result in a performance drop if
     * RecyclerView
     * is restored.
     *
     * @param recycleChildrenOnDetach Whether children should be recycled in detach
     *                                or not.
     */
     ...
     * This behaves similar to the layout change for RTL views. When set to true,
     * first item is
     ...
     * of items while smooth scrolling and 0 otherwise. You can override this method
     * to implement
     * your custom layout pre-cache logic.
     * </p>
     *
     * <p>
     * <strong>Note:</strong>Laying out invisible elements generally comes with
     * significant
     * performance cost. It's typically only desirable in places like smooth
     * scrolling to an unknown
     * location, where 1) the extra content helps LinearLayoutManager know in
     * advance when its
     * target is approaching, so it can decelerate early and smoothly and 2) while
     * motion is
     * continuous.
     * </p>
     *
     * <p>
     * Extending the extra layout space is especially expensive if done while the
     * user may change
     * scrolling direction. Changing direction will cause the extra layout space to
     * swap to the
     * opposite side of the viewport, incurring many rebinds/recycles, unless the
     * cache is large
     * enough to handle it.
     * </p>
     *
     * @return The extra space that should be laid out (in pixels).
     * @deprecated Use {@link #calculateExtraLayoutSpace(RecyclerView.State, int[])}
     * instead.
     */
     ...
     * {@link
     * LinearLayoutManager} and stores the result in {@code extraLayoutSpace}.
     * {@code
     * extraLayoutSpace[0]} should be used for the extra space at the top/left, and
     * {@code
     * extraLayoutSpace[1]} should be used for the extra space at the bottom/right
     * (depending on the
     ...
     *
     * <p>
     * By default, {@code LinearLayoutManager} lays out 1 extra page of items while
     * smooth
     * scrolling, in the direction of the scroll, and no extra space is laid out in
     * all other
     * situations. You can override this method to implement your own custom
     * pre-cache logic. Use
     * {@link RecyclerView.State#hasTargetScrollPosition()} to find out if a smooth
     * scroll to a
     * position is in progress, and
     * {@link RecyclerView.State#getTargetScrollPosition()} to find out
     * which item it is scrolling to.
     * </p>
     *
     * <p>
     * <strong>Note:</strong>Laying out extra items generally comes with significant
     * performance
     * cost. It's typically only desirable in places like smooth scrolling to an
     * unknown location,
     * where 1) the extra content helps LinearLayoutManager know in advance when its
     * target is
     * approaching, so it can decelerate early and smoothly and 2) while motion is
     * continuous.
     * </p>
     *
     * <p>
     * Extending the extra layout space is especially expensive if done while the
     * user may change
     * scrolling direction. In the default implementation, changing direction will
     * cause the extra
     * layout space to swap to the opposite side of the viewport, incurring many
     * rebinds/recycles,
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        // layout 算法
        // layout algorithm:
        // 1) by checking children and other variables, find an anchor coordinate and an
        // anchor
        // item position.
        ...
        if (focused != null) {
            Log.e(TAG, "layout-focused view position" + getPosition(focused));
        } else {
            Log.e(TAG, "layout-focused view position---" + "null");
        }
        ...
        if (!mAnchorInfo.mValid || mPendingScrollPosition != RecyclerView.NO_POSITION
            ...
            mAnchorInfo.mValid = true;
                ...
                && (mOrientationHelper.getDecoratedStart(focused) >= mOrientationHelper.getEndAfterPadding()
                || mOrientationHelper.getDecoratedEnd(focused) <= mOrientationHelper.getStartAfterPadding())) {
            // This case relates to when the anchor child is the focused view and due to
            // layout
            // shrinking the focused view fell outside the viewport, e.g. when soft keyboard
            // shows
            // up after tapping an EditText which shrinks RV causing the focused view (The
            // tapped
            // EditText which is the anchor child) to get kicked out of the screen. Will
            // update the
            // anchor coordinate in order to make sure that the focused view is laid out.
            // Otherwise,
            // the available space in layoutState will be calculated as negative preventing
            // the
            // focused view from being laid out in fill.
            // Note that we won't update the anchor position between layout passes (refer to
            // TestResizingRelayoutWithAutoMeasure), which happens if we were to call
            // updateAnchorInfoForLayout for an anchor that's not the focused view (e.g. a
            // reference
        ...
        // LLM may decide to layout items for "extra" pixels to account for scrolling
        // target,
        ...
        // TODO we can probably avoid this if neither stackFromEnd/reverseLayout/RTL
        // values have
        // changed
        if (getChildCount() > 0) {
    ...
    /**
     * Method called when Anchor position is decided. Extending class can setup
     * accordingly or
     * even update anchor info if necessary.
     *
     * @param recycler                 The recycler for the layout
     * @param state                    The layout state
     * @param anchorInfo               The mutable POJO that keeps the position and
     *                                 offset.
     * @param firstLayoutItemDirection The direction of the first layout filling in
     *                                 terms of adapter
     *                                 indices.
     */
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler,
                                               ...
                                               int endOffset) {
        // If there are scrap children that we did not layout, we need to find where
        // they did go
        // and layout them accordingly so that animations can work as expected.
        // This case may happen if new views are added or an existing view expands and
        // pushes
    ...
    private void updateAnchorInfoForLayout(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        Log.e(TAG, "onLayout-确定锚点信息" + anchorInfo.toString());
    }

    /**
     * Finds an anchor child from existing Views. Most of the time, this is the view
     * closest to
    ...
    private boolean updateAnchorFromChildren(RecyclerView.Recycler recycler,
        ...
        View referenceChild = findReferenceChild(
                recycler,
                state,
        ...
        if (referenceChild != null) {
            ...
            // If all visible views are removed in 1 pass, reference child might be out of
            // bounds.
            // If that is the case, offset it back to 0 so that we use these pre-layout
            // children.
    ...
    /**
     * If there is a pending scroll position or saved states, updates the anchor
     * info from that
    ...
    private boolean updateAnchorFromPendingData(RecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        // if child is visible, try to make it a reference child and ensure it is fully
        // visible.
     ...
     *
     * <p>
     * Note that scroll position change will not be reflected until the next layout
     * call.
     * </p>
     *
    ...
    /**
     * Scroll to the specified adapter position with the given offset from resolved
     * layout
     ...
     * <p>
     * For example, if layout is {@link #VERTICAL} and {@link #getStackFromEnd()} is
     * true, calling
     ...
     * @param offset   The distance (in pixels) between the start edge of the item
     *                 view and
     ...
     * thumb is computed
     * based on the number of visible pixels in the visible items. This however
     * assumes that all
     * list items have similar or equal widths or heights (depending on list
     * orientation).
     * If you use a list in which items have different dimensions, the scrollbar
     * will change
     * appearance as the user scrolls through the list. To avoid this issue, you
     * need to disable
     * this property.
     * <p>
     * When smooth scrollbar is disabled, the position and size of the scrollbar
     * thumb is based
     * solely on the number of items in the adapter and the position of the visible
     * items inside
     * the adapter. This provides a stable scrollbar as the user navigates through a
     * list of items
    ...
    /**
     * Returns the current state of the smooth scrollbar feature. It is enabled by
     * default.
     *
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
        ...
        if (layoutToEnd) {
            ...
            // calculate how much we can scroll without adding new children (independent of
            // layout)
     ...
     * {@link #collectInitialPrefetchPositions(int, LayoutPrefetchRegistry)}, which
     * defines
     * how many inner items should be prefetched when this LayoutManager's
     * RecyclerView
     * is nested inside another RecyclerView.
     *
     * <p>
     * Set this values to the number of items this inner LayoutManager will display
     * when it is
     * first scrolled into the viewport. RecyclerView will attempt to prefetch that
     * number of items
     * so they are ready, avoiding jank as the inner RecyclerView is scrolled into
     * the viewport.
     * </p>
     *
     * <p>
     * For example, take a vertically scrolling RecyclerView with horizontally
     * scrolling inner
     * RecyclerViews. The rows always have 4 items visible in them (or 5 if not
     * aligned). Passing
     * <code>4</code> to this method for each inner RecyclerView's
     * LinearLayoutManager will enable
     * RecyclerView's prefetching feature to do create/bind work for 4 views within
     * a row early,
     * before it is scrolled on screen, instead of just the default 2.
     * </p>
     *
     * <p>
     * Calling this method does nothing unless the LayoutManager is in a
     * RecyclerView
     * nested in another RecyclerView.
     * </p>
     *
     * <p class="note">
     * <strong>Note:</strong> Setting this values to be larger than the number of
     * views that will be visible in this view can incur unnecessary bind work, and
     * an increase to
    ...
    /**
     * Recycles views that went out of bounds after scrolling towards the end of the
     * layout.
     * <p>
     * Checks both layout position and visible position to guarantee that the view
     * is not visible.
     *
     * @param recycler        Recycler instance of {@link RecyclerView}
     * @param scrollingOffset This can be used to add additional padding to the
     *                        visible area. This
     *                        is used to detect children that will go out of bounds
     *                        after scrolling,
     *                        without actually moving them.
     * @param noRecycleSpace  Extra space that should be excluded from recycling.
     *                        This is the space
    ...
    private void recycleViewsFromStart(RecyclerView.Recycler recycler, int scrollingOffset,
        ...
        if (scrollingOffset < 0) {
            if (DEBUG) {
                Log.d(TAG, "Called recycle from start with a negative values. This might happen"
    ...
    private void recycleViewsFromEnd(RecyclerView.Recycler recycler, int scrollingOffset,
        ...
        if (scrollingOffset < 0) {
            if (DEBUG) {
                Log.d(TAG, "Called recycle from end with a negative values. This might happen"
                        + " during layout changes but may be sign of a bug");
            }
            return;
        }
    ...
    /**
     * Helper method to call appropriate recycle method depending on current layout
     * direction
     *
     * @param recycler    Current recycler that is attached to RecyclerView
     * @param layoutState Current layout state. Right now, this object does not
     *                    change but
     *                    we may consider moving it out of this view so passing
     *                    around as a
    ...
    /**
     * The magic functions :). Fills the given layout, defined by the layoutState.
     * This is fairly
     * independent from the rest of the {@link LinearLayoutManager}
     * and with little change, can be made publicly available as a helper class.
     *
     * @param recycler        Current recycler that is attached to RecyclerView
     * @param layoutState     Configuration on how we should fill out the available
     *                        space.
     * @param state           Context passed by the RecyclerView to control scroll
     *                        steps.
...
//        log("start fill");
        ...
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
            ...
            if (!layoutChunkResult.mIgnoreConsumed || layoutState.mScrapList != null
                ...
                // we keep a separate remaining space because mAvailable is important for
                // recycling
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        // We calculate everything with View's bounding box (which includes decor and
        // margins)
     ...
     * @return {@link LayoutState#LAYOUT_START} or {@link LayoutState#LAYOUT_END} if
     * focus direction
     * is applicable to current state, {@link LayoutState#INVALID_LAYOUT}
     * otherwise.
     */
    ...
    /**
     * Convenience method to find the child closes to start. Caller should check it
     * has enough
     * children.
     *
    ...
    /**
     * Convenience method to find the child closes to end. Caller should check it
     * has enough
     * children.
     *
    ...
    /**
     * Convenience method to find the visible child closes to start. Caller should
     * check if it has
     * enough children.
     *
     * @param completelyVisible Whether child should be completely visible or not
     * @return The first visible child closest to start of the layout from user's
     * perspective.
     */
    ...
    /**
     * Convenience method to find the visible child closes to end. Caller should
     * check if it has
     * enough children.
     *
     * @param completelyVisible Whether child should be completely visible or not
     * @return The first visible child closest to end of the layout from user's
     * perspective.
     */
     ...
     * <p>
     * Due to ambiguous adapter updates or children being removed, some children's
     * positions may be
     * invalid. This method is a best effort to find a position within adapter
     * bounds if possible.
     * <p>
     ...
     * </ol>
     *
     * @param layoutFromEnd                  True if the RV scrolls in the reverse
     *                                       direction, which is the same as
     *                                       (reverseLayout ^ stackFromEnd).
     * @param traverseChildrenInReverseOrder True if the children should be
    ...
    // returns the out-of-bound child view closest to RV's end bounds. An
    // out-of-bound child is
    // defined as a child that's either partially or fully invisible (outside RV's
    // padding area).
    ...
    // returns the out-of-bound child view closest to RV's starting bounds. An
    // out-of-bound child is
    // defined as a child that's either partially or fully invisible (outside RV's
    // padding area).
    ...
    /**
     * Returns the adapter position of the first visible view. This position does
     * not include
     * adapter changes that were dispatched after the last layout pass.
     * <p>
     * Note that, this values is not affected by layout orientation or item order
     * traversal.
     * ({@link #setReverseLayout(boolean)}). Views are sorted by their positions in
     * the adapter,
     * not in the layout.
     * <p>
     * If RecyclerView has item decorators, they will be considered in calculations
     * as well.
     * <p>
     * LayoutManager may pre-cache some views that are not necessarily visible.
     * Those views
     * are ignored in this method.
     *
     * @return The adapter position of the first visible item or
     * {@link RecyclerView#NO_POSITION} if
    ...
    /**
     * Returns the adapter position of the first fully visible view. This position
     * does not include
     * adapter changes that were dispatched after the last layout pass.
     * <p>
     * Note that bounds check is only performed in the current orientation. That
     * means, if
     * LayoutManager is horizontal, it will only check the view's left and right
     * edges.
     *
    ...
    /**
     * Returns the adapter position of the last visible view. This position does not
     * include
     * adapter changes that were dispatched after the last layout pass.
     * <p>
     * Note that, this values is not affected by layout orientation or item order
    ...
    /**
     * Returns the adapter position of the last fully visible view. This position
     * does not include
     * adapter changes that were dispatched after the last layout pass.
     * <p>
     * Note that bounds check is only performed in the current orientation. That
    ...
    // Returns the first child that is visible in the provided index range, i.e.
    // either partially or
    // fully visible depending on the arguments provided. Completely invisible
    // children are not
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
        ...
        // nextCandidate is the first child view in the layout direction that's
        // partially
        // within RV's bounds, i.e. part of it is visible or it's completely invisible
        // but still
        // touching RV's bounds. This will be the unfocusable candidate view to become
        // visible onto
        ...
        // nextFocus is meaningful only if it refers to a focusable child, in which case
        // it
    ...
    /**
     * Used for debugging.
     * Validates that child views are laid out in correct order. This is important
     * because rest of
     ...
     * In reverse layout, last child should be closes to screen position 0 and first
     * child should
    ...
    /**
     * Helper class that keeps temporary state while {LayoutManager} is filling out
     * the empty
     * space.
     */
    static class LayoutState {
         ...
         * It should be set the amount of scrolling we can make without creating a new
         * view.
         ...
         * The difference with {@link #mAvailable} is that, when recycling, distance
         * laid out for
         * {@link #mExtraFillSpace} is not considered to avoid recycling visible
         * children.
         */
        int mExtraFillSpace = 0;

        /**
         * Contains the {@link #calculateExtraLayoutSpace(RecyclerView.State, int[])}
         * extra layout
         * space} that should be excluded for recycling when cleaning up the tail of the
         * list during
         * a smooth scroll.
         */
        ...
        /**
         * Equal to {@link RecyclerView.State#isPreLayout()}. When consuming scrap, if
         * this values
         * is set to true, we skip removed views since they should not be laid out in
         * post layout
         * step.
         */
        ...
        /**
         * The most recent
         * {@link #scrollBy(int, RecyclerView.Recycler, RecyclerView.State)}
         * amount.
         */
        int mLastScrollDelta;

        /**
         * When LLM needs to layout particular views, it sets this list in which case,
         * LayoutState
         * will only return views from this list and return null if it cannot find an
         * item.
         */
         ...
         * Also updates current item index to the next item, based on
         * {@link #mItemDirection}
         *
         ...
         * <p>
         * Upon finding a valid VH, sets current item position to VH.itemPosition +
         * mItemDirection
         *
         * @return View if an item in the current position or direction exists if not
         * null.
         */
    ...
    static class AnchorInfo {
        ...
        /**
         * assigns anchor coordinate from the RecyclerView's padding depending on
         * current
         * layoutFromEnd values
         */
