...
public class RecyclerView extends ViewGroup implements ScrollingView,
    ...
    static final String TAG_MSH = "MSH_RecyclerView";
     ...
     * this values might be garbage. To save LayoutManagers from it, RecyclerView sets the size to
    ...
    // This values is used when handling rotary encoder generic motion events.
        ...
        switch (slopConstant) {
            default:
                Log.w(TAG, "setScrollingTouchSlop(): bad argument constant "
                        + slopConstant + "; using default values");
                // fall-through
     ...
     * @param index Position in the decoration chain to insert this decoration at. If this values
     ...
     * range. This values is used to compute the length of the thumb within the scrollbar's track.
     * </p>
     *
     ...
     * horizontal range. This values is used to compute the length of the thumb within the
     * scrollbar's track.</p>
     *
     * <p>The range is expressed in arbitrary units that must be the same as the units used by
     ...
     * This values is used to compute the length of the thumb within the scrollbar's track. </p>
     *
     * <p>The range is expressed in arbitrary units that must be the same as the units used by
     ...
     *   {@link LayoutManager} must support scrolling in a direction for which the values is not 0.
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        log("onMeasuer()");
        //如果不配置LayoutManager 调用 默认的测量, item不会显示
        if (mLayout == null) {
        ...
        if (mLayout.isAutoMeasureEnabled()) {
            ...
            log("判断是否是 精准的宽高 mLastAutoMeasureSkippedDueToExact--->" + mLastAutoMeasureSkippedDueToExact);
            //如果 RecyclerView 宽高是 精准的值 到此结束
            if (mLastAutoMeasureSkippedDueToExact || mAdapter == null) {
                log("mLastAutoMeasureSkippedDueToExact");
                return;
            }
            log("onMeasure finish because mLastAutoMeasureSkippedDueToExact");
            //当宽高 不是最大宽高, 后续继续计算
            ...
            if (mLayout.shouldMeasureTwice()) {
                log("onMeasure-shouldMeasureTwice");
            ...
            log("isAutoMeasureEnabled = false");
            //自定义layoutManager
            if (mHasFixedSize) {
                mLayout.onMeasure(mRecycler, mState, widthSpec, heightSpec);
                return;
    ...
    /**
     * Gets the current ItemAnimator for this RecyclerView. A null return values
    ...
    private void processAdapterUpdatesAndSetAnimationFlags() {
        if (mDataSetHasChangedAfterLayout) {
            // Processing these items have no values since data set changed unexpectedly.
    ...
    void dispatchLayout() {
        ...
        if (mState.mLayoutStep == State.STEP_START) {
            ...
            // changed size.
            // 前两步 其实已经在onMeasure 执行过了, 但是如果RecyclerVIew 大小改变了 或者adapte 更新了 需要重新执行一遍
    ...
    private void dispatchLayoutStep1() {
        log("dispatchLayoutStep11111111111111111111");
        mState.assertLayoutStep(State.STEP_START);
        //计算RecyclerView 滑动偏移量
        fillRemainingScrollValues(mState);
        mState.mIsMeasuring = false;
        startInterceptRequestLayout();
        ...
        Log.e(TAG, "dispatchLayoutStep1--->" + mState.toString());
        if (mState.mRunSimpleAnimations) {
            // Step 0: Find out where all non-removed items are, pre-layout
            //：找出所有未删除的项目的位置，预布局
            int count = mChildHelper.getChildCount();
            Log.e(TAG, "mState.mRunSimpleAnimations");
            for (int i = 0; i < count; ++i) {
                final ViewHolder holder = getChildViewHolderInt(mChildHelper.getChildAt(i));
        ...
        if (mState.mRunPredictiveAnimations) {
            ...
            //步骤1：运行预布局：这将使用项目的旧位置。布局管理器
            //预计会布局所有内容，甚至是删除的项目（尽管不会添加删除的项目
            //物品返回容器）。这给出了显示视图的预布局位置
            //作为真实布局的一部分而存在。
            //保存旧位置，以便LayoutManager可以运行其映射逻辑。
            saveOldPositions();
    ...
    private void dispatchLayoutStep2() {
        log("dispatchLayoutStep222222222222");
        startInterceptRequestLayout();
        onEnterLayoutOrScroll();
        ...
        // Step 2: Run layout
        mState.mInPreLayout = false;
        //对子view进行绘制 真正的 child layout  linearlayoutManager
        mLayout.onLayoutChildren(mRecycler, mState);
     ...
     * *布局的最后一步是保存有关动画视图的信息，*触发动画并执行任何必要的清理。
     */
    private void dispatchLayoutStep3() {
        log("dispatchLayoutStep333333333333333");
     ...
     * <p>
     * By default, this values is {@code true}.
     *
     ...
     * next layout calculation. If there are pending adapter updates, the return values of this
    ...
    public final class Recycler {
         ...
         * return -1.
         * <p>
         * Calling this method in post-layout state returns the same values back.
         *
...
//            log("tryGetViewHolderForPositionByDeadline--->" + position);
            if (position < 0 || position >= mState.getItemCount()) {
            ...
            if (holder == null) {
                ...
                if (holder == null) {
                    ...
                    log("no cached and create new holder "+ position);
    ...
    public abstract static class Adapter<VH extends ViewHolder> {
         ...
         * @return integer values identifying the type of the view needed to represent the item at
        ...
        /**
         * Returns true if this adapter publishes a unique <code>long</code> values that can
         ...
         * values of this method to decide whether the View should be recycled or not.
         * <p>
         ...
         * the return values of {@link #getStateRestorationPolicy()}.
         *
    ...
    public abstract static class LayoutManager {
         ...
         * then caps the values to be within the given measurement specs.
         *
         ...
         * {@link LayoutManager#onMeasure(Recycler, State, int, int)}.
         * <p>
         * This method returns false by default (it actually returns the values passed to the
         ...
         * different values. For instance, GridLayoutManager overrides this values to handle the case
         ...
         * the return values from this method and the
         ...
         * view system traversals on devices running API 21 or greater. Default values is true.</p>
         *
         ...
         * false otherwise. When the returned values is true, the Runnable
         ...
         *                 position
         * @param state    Transient state of RecyclerView
         * @return The actual distance scrolled. The return values will be negative if dx was
         ...
         *                 position
         * @param state    Transient state of RecyclerView
         * @return The actual distance scrolled. The return values will be negative if dy was
         * negative and scrolling proceeeded in that direction.
         ...
         *
         * <p>This values is set only if the LayoutManager opts into the AutoMeasure api via
         * {@link #setAutoMeasureEnabled(boolean)}.
         *
         * <p>When RecyclerView is running a layout, this values is always set to
         ...
         *
         * <p>This values is usually equal to the laid out width of the {@link RecyclerView} but may
         ...
         *
         * <p>This values is usually equal to the laid out height of the {@link RecyclerView} but may
        ...
        /**
         * Calculate a MeasureSpec values for measuring a child view in one dimension.
         *
         ...
         * @return a MeasureSpec values for the child view
         ...
         * <p>
         * Note that this values is not updated until the View is measured or
    ...
    public abstract static class ViewHolder {
        ...
        // Saves isImportantForAccessibility values for the view item while it's in hidden state and
         ...
         * next layout pass, the return values of this method will be {@link #NO_POSITION}.
         * <p>
         ...
         * reference-counted.
         *
         * @param recyclable Whether this item is available to be recycled. Default values
         *                   is true.
         * @see #isRecyclable()
         */
    ...
    public static class State {
        ...
        /**
         * Returns true if the RecyclerView is currently measuring the layout. This values is
         ...
         * pre-layout step, this values will be {@code false} even if the RecyclerView is in
        ...
        /**
         * Adds a mapping from the specified id to the specified values, replacing the previous
        ...
        /**
         * If scroll is triggered to make a certain item visible, this values will return the
